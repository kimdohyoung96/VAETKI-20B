...
def remove_think_block(text):
    """
    다음 턴 history 누적 시 <think>...</think> 제거
    """
    return re.sub(
        r"<think>.*?</think>",
        "",
    ...
    messages = [
        {
            ...
            "content": (
                ...
                "이전 대화의 조건을 반영하되, 답변은 간결하고 명확하게 작성하세요. "
                "같은 문장을 반복하지 마세요."
            )
        }
    ]
    ...
    for turn_idx, user_prompt in enumerate(turns, 1):
        ...
        try:
            r = client.chat.completions.create(
                ...
                # reasoning runaway 줄이기 위해 stop token 추가
                # 1, 2 둘 중 하나만 사용
                # stop=["<|END|>"]
                # stop=["</think>"]
            )
            ...
            # 다음 turn history에는 think 제거한 final response만 누적
            messages.append(
                {
                    "role": "assistant",
