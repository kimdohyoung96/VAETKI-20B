...
Added for long-turn experiments:
  ...
  - no-empty fallback: if visible answer is empty after think stripping, keep sanitized think text
...
def dedupe_repeated_answer(text: str) -> str:
    """Remove duplicated final answers more aggressively.

    Handles two common VAETKI failure patterns:
    1) The same final answer block appears twice after malformed </think> tags.
    2) Similar paragraph/list blocks are repeated with small wording changes.

    The function is still heuristic-based, so it avoids deleting short answers.
    """
    if not text:
        return text
    ...
    # 1) Exact paragraph-level duplicate removal.
    paragraphs = [p.strip() for p in re.split(r"\n\s*\n", text) if p.strip()]
    deduped_paragraphs: List[str] = []
    seen_keys = set()

    for p in paragraphs:
        # Normalize whitespace and trim very long paragraphs into a stable key.
    ...
    # 2) If the whole answer is roughly duplicated as first half + second half,
    # keep the second half. This is useful when the second block is the clean
    # final answer after a repeated </think> boundary.
    n = len(text)
    if n >= 500:
        mid = n // 2
    ...
    # 3) Marker-based block truncation. If the same answer-start marker appears
    # twice and the second block is long, keep only the first complete block.
    # This catches repeated headings/lists such as:
    #   다음은 ...
    #   1. ...
    #   ...
    #   다음은 ...
    ...
    for marker in markers:
        ...
        # Avoid over-cutting tiny repeated words. Only cut if the repeated block
        # after the second marker is substantial.
        if len(before) >= 250 and len(after) >= 250:
            return before

    # 4) Sentence-level near-duplicate cleanup for repeated closing sentences.
    # This helps when the ending like "궁금한 점이 있으면..." repeats.
...
def extract_visible_answer(reply: str) -> str:
    """Extract only visible answer after removing <think> blocks.

    This strict cleaner can become empty when the model places the entire
    response inside <think>...</think> and does not generate a visible answer.
    """
    if not reply:
        return ""
...
def extract_sanitized_think_fallback(reply: str) -> str:
    """Fallback for EMPTY outputs caused by over-cleaning think-only replies.

    If useful generated text exists only inside <think>...</think>, keep that
    text after removing literal think tags. This reduces EMPTY outputs while
    avoiding raw </think> tag contamination in CSV/history.
    """
    if not reply:
        return ""

    fallback = re.sub(r"</?think>", "", reply).strip()
    fallback = dedupe_repeated_answer(fallback)
    return fallback.strip()


def clean_reply_for_storage(reply: str) -> Tuple[str, str]:
    """Return final assistant text saved to CSV/history and its source type.

    source_type:
      - visible: normal visible answer after removing paired think blocks
      - think_fallback: no visible answer existed, so sanitized think content was kept
      - empty: nothing usable was generated
    """
    visible = extract_visible_answer(reply)
    if visible:
        return visible, "visible"

    fallback = extract_sanitized_think_fallback(reply)
    if fallback:
        return fallback, "think_fallback"
...
def ensure_columns(rows: List[Dict[str, str]], fieldnames: List[str]) -> None:
    required = [
        ...
        "generated_conversation_raw_json",
        "turn_latencies_ms_a",
        "num_turns_a",
...
def main() -> None:
    ...
    for idx, row in enumerate(rows, start=1):
        ...
        generated_turns_raw: List[Dict[str, str]] = []
        any_error = False
        first_collapse_turn = ""
        first_collapse_type = ""
        ...
        for turn_i, user_msg in enumerate(user_turns, start=1):
            ...
            generated_turns_raw.append({"role": "user", "content": user_msg})
            ...
            if err:
                ...
                generated_turns_raw.append({"role": "assistant", "content": ""})
            ...
            cleaned_reply, clean_source = clean_reply_for_storage(reply)
            ...
            # Important: keep the cleaned reply in CSV even when it is dropped from history.
            ...
            # Save the cleaned assistant reply to the output CSV. If the model only
            # generated text inside <think>, the no-empty fallback stores sanitized
            # think text instead of writing an empty answer.
            generated_turns.append({"role": "assistant", "content": cleaned_reply})
            generated_turns_raw.append({"role": "assistant", "content": reply})
            ...
            empty_marker = " [EMPTY-skipped from history]" if not history_reply else ""
            print(
                f"[{idx}/{total}] {sid} "
        ...
        row["model_a_raw_output"] = render_conversation(generated_turns_raw)
        ...
        row["generated_conversation_raw_json"] = json.dumps(generated_turns_raw, ensure_ascii=False)
