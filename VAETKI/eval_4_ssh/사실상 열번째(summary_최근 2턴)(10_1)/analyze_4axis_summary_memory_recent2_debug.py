    ...
    with open(path, "r", encoding="utf-8-sig", newline="") as f:
        for row in csv.DictReader(f):
            try:
                ...
                conv = []
            lats = _safe_json_list(row.get("turn_latencies_ms_a", "[]"))
            prompt_tokens = _safe_json_list(row.get("turn_prompt_tokens_a", "[]"))
            memory_tokens = _safe_json_list(row.get("turn_memory_tokens_a", "[]"))
            reply_tokens = _safe_json_list(row.get("turn_reply_tokens_a", "[]"))
            clean_tokens = _safe_json_list(row.get("turn_clean_tokens_a", "[]"))
            statuses = _safe_json_list(row.get("turn_status_a", "[]"))
            clean_sources = _safe_json_list(row.get("turn_clean_source_a", "[]"))
            user_turns: List[str] = []
            ...
            for i, (u, a) in enumerate(zip(user_turns, assistant_turns), start=1):
                ...
                classified.append({
                    ...
                    "prompt_tokens": prompt_tokens[i - 1] if i - 1 < len(prompt_tokens) else None,
                    "memory_tokens": memory_tokens[i - 1] if i - 1 < len(memory_tokens) else None,
                    "reply_tokens": reply_tokens[i - 1] if i - 1 < len(reply_tokens) else None,
                    "clean_tokens": clean_tokens[i - 1] if i - 1 < len(clean_tokens) else None,
                    "debug_status": statuses[i - 1] if i - 1 < len(statuses) else None,
                    "clean_source": clean_sources[i - 1] if i - 1 < len(clean_sources) else None,
                })
            ...
            sessions.append({
                ...
                "turns": classified,
                "history_strategy": row.get("history_strategy_a", ""),
                "summary_memory": row.get("summary_memory_a", ""),
                "max_prompt_tokens": row.get("max_prompt_tokens_a", ""),
                "first_collapse_turn": row.get("first_collapse_turn_a", ""),
                "first_collapse_type": row.get("first_collapse_type_a", ""),
            })
    return sessions
...
def render_report(sessions: List[Dict], mode: str) -> str:
    ...
    # Summary-memory diagnostics if present
    prompt_vals = [t.get("prompt_tokens") for t in all_turns if isinstance(t.get("prompt_tokens"), (int, float))]
    memory_vals = [t.get("memory_tokens") for t in all_turns if isinstance(t.get("memory_tokens"), (int, float))]
    reply_vals = [t.get("reply_tokens") for t in all_turns if isinstance(t.get("reply_tokens"), (int, float))]
    source_vals = Counter(t.get("clean_source") for t in all_turns if t.get("clean_source"))
    debug_vals = Counter(t.get("debug_status") for t in all_turns if t.get("debug_status"))
    if prompt_vals or source_vals or debug_vals:
        lines.append("## 1-1. Summary Memory / Recent-Turn 진단")
        lines.append("")
        if prompt_vals:
            lines.append(f"- prompt token 중앙값: {median(prompt_vals):.0f}")
            lines.append(f"- prompt token 최대값: {max(prompt_vals):.0f}")
        if memory_vals:
            lines.append(f"- memory token 중앙값: {median(memory_vals):.0f}")
            lines.append(f"- memory token 최대값: {max(memory_vals):.0f}")
        if reply_vals:
            lines.append(f"- reply token 중앙값: {median(reply_vals):.0f}")
            lines.append(f"- reply token 최대값: {max(reply_vals):.0f}")
        if source_vals:
            lines.append("- clean source 분포: " + ", ".join(f"{k}={v}" for k, v in source_vals.most_common()))
        if debug_vals:
            lines.append("- debug status 분포: " + ", ".join(f"{k}={v}" for k, v in debug_vals.most_common()))
        lines.append("")
    ...
    for s in sessions:
        ...
        lines.append("")
        lines.append("| Turn | User (요약) | 분류 | Debug | Source | Prompt tok | Reply tok | 답 길이 | latency(s) |")
        lines.append("|---|---|---|---|---|---:|---:|---:|---:|")
        for t in s["turns"]:
            ...
            debug = t.get("debug_status") or "-"
            source = t.get("clean_source") or "-"
            ptok = t.get("prompt_tokens") if t.get("prompt_tokens") is not None else "-"
            rtok = t.get("reply_tokens") if t.get("reply_tokens") is not None else "-"
            lines.append(
                f"| {t['turn_idx']} | {user_short} | {cls_short} | {debug} | {source} | {ptok} | {rtok} | {len(t['final_text'])} | {lat} |"
            )
        lines.append("")
