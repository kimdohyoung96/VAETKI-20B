...
Reads a multi-turn run CSV (output of run_multiturn_generative.py or run_multiturn_generative_summary_memory.py) over the
    ...
    with open(path, "r", encoding="utf-8-sig", newline="") as f:
        for row in csv.DictReader(f):
            try:
                ...
                conv = []
            def _load_json_list(col: str) -> List:
                try:
            ...
            lats = _load_json_list("turn_latencies_ms_a")
            prompt_chars = _load_json_list("turn_prompt_chars_a")
            memory_chars = _load_json_list("turn_memory_chars_a")
            raw_chars = _load_json_list("turn_raw_chars_a")
            clean_chars = _load_json_list("turn_clean_chars_a")
            user_turns: List[str] = []
            ...
            for i, (u, a) in enumerate(zip(user_turns, assistant_turns), start=1):
                ...
                classified.append({
                    ...
                    "prompt_chars": prompt_chars[i - 1] if i - 1 < len(prompt_chars) else None,
                    "memory_chars": memory_chars[i - 1] if i - 1 < len(memory_chars) else None,
                    "raw_chars": raw_chars[i - 1] if i - 1 < len(raw_chars) else None,
                    "clean_chars": clean_chars[i - 1] if i - 1 < len(clean_chars) else None,
                })
...
def render_report(sessions: List[Dict], mode: str) -> str:
    ...
    # Summary-memory / context diagnostics
    prompt_vals = [t.get("prompt_chars") for t in all_turns if t.get("prompt_chars") is not None]
    memory_vals = [t.get("memory_chars") for t in all_turns if t.get("memory_chars") is not None]
    raw_vals = [t.get("raw_chars") for t in all_turns if t.get("raw_chars") is not None]
    clean_vals = [t.get("clean_chars") for t in all_turns if t.get("clean_chars") is not None]
    if prompt_vals or memory_vals or raw_vals or clean_vals:
        lines.append("## 3. Summary Memory / Context 진단")
        lines.append("")
        if prompt_vals:
            lines.append(f"- prompt chars 중앙값: {median(prompt_vals):.0f}")
            lines.append(f"- prompt chars 최대값: {max(prompt_vals):.0f}")
        if memory_vals:
            lines.append(f"- memory chars 중앙값: {median(memory_vals):.0f}")
            lines.append(f"- memory chars 최대값: {max(memory_vals):.0f}")
        if raw_vals:
            lines.append(f"- raw output chars 중앙값: {median(raw_vals):.0f}")
            lines.append(f"- raw output chars 최대값: {max(raw_vals):.0f}")
        if clean_vals:
            lines.append(f"- clean output chars 중앙값: {median(clean_vals):.0f}")
            lines.append(f"- clean output chars 최대값: {max(clean_vals):.0f}")
        lines.append("")

    # Failure distribution
    lines.append("## 4. 실패 유형 분포")
    lines.append("")
    lines.append("### 4.1 일반 실패")
    ...
    lines.append("")
    lines.append("### 4.2 4축 특화 실패 (generic=SUCCESS이지만 트랙 기준 미충족)")
    ...
    # Per-session matrix
    lines.append("## 5. 세션별 turn 결과")
    lines.append("")
    for s in sessions:
        lines.append(f"### {s['session_id']} ({s['task_track']} / {s['task_subtype']})")
        lines.append("")
        lines.append("| Turn | User (요약) | 분류 | 답 길이 | latency(s) | prompt chars | memory chars |")
        lines.append("|---|---|---|---|---|---:|---:|")
        for t in s["turns"]:
            ...
            prompt_c = t.get("prompt_chars")
            memory_c = t.get("memory_chars")
            prompt_s = str(prompt_c) if prompt_c is not None else "-"
            memory_s = str(memory_c) if memory_c is not None else "-"
            lines.append(
                f"| {t['turn_idx']} | {user_short} | {cls_short} | {len(t['final_text'])} | {lat} | {prompt_s} | {memory_s} |"
            )
        lines.append("")
